...
func (p *MediaGalleryMongoDBDao) InitializeDao(Client utils.Map, businessId string) {
	log.Println("Initialaze MediaGallery Mongodb DAO")
	p.Client = Client
...
func (t *MediaGalleryMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("MediaGallery Save - Begin ", indata)
	// sites MediaGallery
	collection, ctx, err := mongo_utils.GetMongoDbCollection(t.Client, sites_common.Dbmedia_gallery)
	if err != nil {
	...
	log.Println("Save - End ", indata[sites_common.FLD_MEDIA_GALLERY_ID])

	return t.Get(indata[sites_common.FLD_MEDIA_GALLERY_ID].(string))
}
...
func (t *MediaGalleryMongoDBDao) Get(mediagalleryId string) (utils.Map, error) {
	...
	log.Println("MediaGalleryMongoDBDao::Get - Begin ", mediagalleryId)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(t.Client, sites_common.Dbmedia_gallery)
	log.Println("Get :: Got collection")

	filter := bson.D{{Key: sites_common.FLD_MEDIA_GALLERY_ID, Value: mediagalleryId}, {}}
	...
	log.Printf("Business MediaGalleryMongoDBDao :: Get:: End Found a single document: %+v\n", result)
	return result, nil
}
...
func (t *MediaGalleryMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin  - Find All Collection Dao ", sites_common.Dbmedia_gallery)
...
func (t *MediaGalleryMongoDBDao) Update(mediagalleryId string, indata utils.Map) (utils.Map, error) {
	...
	filterMediaGallery := bson.D{{Key: sites_common.FLD_MEDIA_GALLERY_ID, Value: mediagalleryId}}
	updateResult1, err := collection.UpdateOne(ctx, filterMediaGallery, bson.D{{Key: "$set", Value: indata}})
	if err != nil {
		return utils.Map{}, err
	}
	...
	return t.Get(mediagalleryId)
}

// Delete - Delete Collection
func (t *MediaGalleryMongoDBDao) Delete(mediagalleryId string) (int64, error) {

	log.Println("MediaGallery MongoDBDao::Delete - Begin ", mediagalleryId)
	...
	optsMediaGallery := options.Delete().SetCollation(&options.Collation{
	...
	filterMediaGallery := bson.D{{Key: sites_common.FLD_MEDIA_GALLERY_ID, Value: mediagalleryId}}
	resmediagallery, err := collection.DeleteOne(ctx, filterMediaGallery, optsMediaGallery)
	if err != nil {
		log.Println("Error in delete ", err)
		return 0, err
	}
	log.Printf("MediaGallery MongoDBDao::Delete - End deleted %v documents\n", resmediagallery.DeletedCount)
	return resmediagallery.DeletedCount, nil
}
...
func (p *MediaGalleryMongoDBDao) Find(filter string) (utils.Map, error) {
	...
	log.Println("MediaGalleryMongoDBDao::Find:: Begin ", filter)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.Client, sites_common.Dbmedia_gallery)
	...
	log.Println("MediaGalleryMongoDBDao::Find:: End Found a single document: \n", err)
	return result, nil
}
